function [p] = plotsurf_wrapper( v, f ,color, do_camlight)
% [p] = PLOTSURF_WRAPPER (v, f, color) : a function to make it easy to plot 
% surfaces as they're pulled out by read_surf. plotsurf is a
% function in the iso2mesh package
%
% http://iso2mesh.sourceforge.net/cgi-bin/index.cgi?Doc
%
% f must be 1-indexed. The information read from read_surf is 0-indexed, so
% pass in f=faces+1, when faces is read from read_surf. 
% ***This is automatically done if you are using read_surf_wrapper
%
% accepts:
%   v : Nx3 matrix of the locations of the vertices
%   f : Mx3 matrix of the indices of the vertices connected to make faces
%   do_camlight : whether or not to turn camlight on
%
% returns
...
p = plotsurf(v, f, 'facecolor',color,'linestyle','none');
...
axis('off');
if ~exist('do_camlight','var') || do_camlight
    camlight;
end
